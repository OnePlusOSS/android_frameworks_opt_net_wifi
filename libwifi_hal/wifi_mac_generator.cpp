...
static void update_wlan_mac_bin(uint8_t *mac, int random) {
    FILE *fb = NULL;
    struct stat st;
    char buf [150];
    int i = 0;
...
void get_mac_from_nv() {
    ...
    if ((stat(NV_MAC_FILE_0, &st) != 0 || st.st_size < 6) &&
        ...
        PLOG(ERROR) << __func__ << ": Get nv mac file failed";
        random = 1;
    } else {
        if ((fd = fopen(NV_MAC_FILE_0, "rb+")) == NULL) {
            PLOG(ERROR) << __func__ << ": Could not open " << NV_MAC_FILE_0;
            if ((fd = fopen(NV_MAC_FILE_1, "rb+")) == NULL) {
                PLOG(ERROR) << __func__ << ": Could not open " << NV_MAC_FILE_1;
                random = 1;
            }
        }
    }
